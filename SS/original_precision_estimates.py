...
temp_chunk.optimizeCameras(fit_f=False, fit_cx=False, fit_cy=False, fit_b1=False, fit_b2=False, fit_k1=False,
                           fit_k2=False, fit_k3=False, fit_k4=False, fit_p1=False, fit_p2=False, fit_p3=False,
                           fit_p4=False)

# Export the sparse point cloud 
temp_chunk.exportPoints(dir_path + 'sparse_pts_reference.ply', normals=False, colors=False,
...
for line_ID in range(0, num_randomisations):
    ...
    out_file = ('{0:04d}'.format(file_idx) + '_MA' + '{0:0.5f}'.format(chunk.marker_location_accuracy[0]) +
                '_PA' + '{0:0.5f}'.format(chunk.marker_projection_accuracy) + '_TA' + '{0:0.5f}'.format(
    ...
    try:
        chunk.saveReference(dir_path + out_gc_file, Metashape.ReferenceFormatCSV,
                            items=Metashape.ReferenceItemsMarkers, )
        chunk.saveReference(dir_path + out_cams_c_file, Metashape.ReferenceFormatCSV,
                            items=Metashape.ReferenceItemsCameras)
    except:
        chunk.saveReference(dir_path + out_gc_file, 'csv')

    # Export the cameras
    chunk.exportCameras(dir_path + out_cam_file, format=Metashape.CamerasFormatXML, projection=crs,
    ...
    # Export the sparse point cloud
    chunk.exportPoints(dir_path + out_file + '_pts.ply', normals=False, colors=False, format=Metashape.PointsFormatPLY,
